<template>
    <div class="page-messenger">
        <PageTitle :title="$L(tabActive==='dialog' ? '历史消息' : '通讯录')"/>
        <div class="messenger-wrapper">
                ...
                <div v-if="tabActive==='dialog' && !dialogSearchKey" class="messenger-nav">
                    <EDropdown
                        ...
                        <EDropdownMenu slot="dropdown" class="messenger-nav-menu">
                            <EDropdownItem v-for="(item, index) in dialogMenus" :key="index[0]" :command="item.type">
                                <div class="messenger-nav-item" :class="{active: dialogActive==item.type}">
                                    <div class="nav-title">全部历史会话</div>
            ...
            <div v-if="activeNum > 0 && routeName === 'manage-aiassistant'" class="messenger-msg">
                ...
                <AIDialogWrapper v-if="windowLandscape && dialogId > 0" :dialogId="dialogId" @on-active="scrollIntoActive" :auto-focus="$A.isDesktop()" location="aiassistant"/>
            </div>
        </div>
    </div>
</template>
...
import AIDialogWrapper from "./components/AIDialogWrapper.vue";
        ...
        updateDialogs(timeout) {
            ...
            if (timeout > -1) {
                this.__updateDialogs = setTimeout(_ => {
                    if (this.tabActive === 'dialog' && this.routeName === 'manage-aiassistant') {

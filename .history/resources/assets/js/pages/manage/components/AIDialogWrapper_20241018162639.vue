...
import AIDialogItem from "./AIDialogItem.vue";
...
import ApproveDetails from "../approve/details.vue";
...
export default {
    ...
    methods: {
        ...
        subMsgListener(unsubscribe = false) {
            ...
            if (unsubscribe) {
                this.$store.dispatch('websocketMsgListener', 'AIDialogWrapper')
            } else {
        ...
        onPrevPage() {
            ...
            this.getMsgs({
                ...
                msg_type: this.msgType,
                prev_id: this.prevId,
                save_before: _ => this.scrollDisabled = true,  //禁用滚动
                ...
                this.$nextTick(() => {
                    ...
                    const reducer = ids.reduce((previousValue, currentId) => {
                        ...
                        return {size: previousSize + scroller.getSize(currentId)}  // 计算当前页面的高度（之前消息的高度+当前消息的高度的偏移量）
                    })
                    this.onToOffset(scroller.getOffset() + reducer.size, true)  //将滚动条滚动到新的偏移量
                });
            }).catch(() => {})
        },
                ...
                onOk: () => {
                    return new Promise((resolve, reject) => {
                        this.$store.dispatch("call", {
                            ...
                            this.goForward({name: 'manage-aiassistant'});
                        }).catch(({msg}) => {
                            reject(msg);
                        });
                    })
        ...
        onScroll(event) {
            ...
            //offset 表示当前滚动的偏移量 // tail 表示滚动到列表底部的距离
            const {offset, tail} = this.scrollInfo();
            //更新滚动状态
        ...
        onRange(range) {
            ...
            for (let i = range.start; i <= range.end; i++) {
                ...
                //相邻消息（向前或向后）存在且其 id 不等于当前加载条件的值
        ...
        onEmoji(data) {
            ...
            //更新缓存， unshift 方法将当前表情符号添加到列表的开头
